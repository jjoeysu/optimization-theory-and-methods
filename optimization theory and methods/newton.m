...
%       梯度的二范数
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[xbar,fxbar,n,b,g]=newton(f,x,epsilon,m)
syms x1 x2
grad=[diff(f,x1);diff(f,x2)];
hf=[diff(grad(1),x1),diff(grad(1),x2);diff(grad(2),x1),diff(grad(2),x2)];
gradx=subs(grad,[x1,x2],[x(1),x(2)]);
hfx=subs(hf,[x1,x2],[x(1),x(2)]);
d=-hfx^(-1)*gradx;k=1;b(k,1:2)=x;g(k,1)=norm(gradx,2);
while(norm(gradx,2)>epsilon&&k<m)
    x=x+d;
    gradx=subs(grad,[x1,x2],[x(1),x(2)]);
    hfx=subs(hf,[x1,x2],[x(1),x(2)]);
    d=-hfx^(-1)*gradx;
    k=k+1;b(k,1:2)=x;g(k,1)=norm(gradx,2);toc;
end
n=k;
